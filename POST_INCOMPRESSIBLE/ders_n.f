...
c      RHS(1  )=(ac*F(1)+bc*F(2  )+cc*F(3  ))*dxi
      ...
      subroutine der1c_s_c6(N,F,dF,dx,Twb,Twt)
c
c
c    |       |       |        |         |
c    1       2       3        4         5
...
c
      ...
      real A(N+1),B(N+1),C(N+1),RHS(N+1),D(N+1)
      real dxi,ac,bc,cc,alpha,fac1,fac2
      real t1(n+1,n+1) ,a1(n+1,n),r(n+1,n),td(n+1,n+1),Twb,Twt
      a1=0
      t1=0
      r=0
      dxi = 1./dx
      ...
      cc = (23-31)/8.
c     RHS(1  )=(ac*F(1)+bc*F(2  )+cc*F(3  ))*dxi
      RHS(1  )=(-15*F(1)+50*F(2  )/3-0.6*F(3 )-16./15.*Twb)*dxi
      A  (N+1  )=15
      B  (N+1  )=1.
      C  (N+1  )=0.
      RHS(N+1  )=(15*f(n)-50*f(n-1)/3+0.6*f(n-2)+16./15.*Twt)*dxi
      call trid(N+1,A,B,C,RHS,D)
      do i=1,N+1
       dF(i-1)=RHS(i)
      enddo
...
c      call trid(N+1,A,B,C,RHS,D)
      ...
      subroutine der1c_s_c6_m(N,NRHS,F,dF,dx,Twb,Twt)
c
c
c    |       |       |        |         |
c    1       2       3        4         5
...
c      
      ...
      do j=1,nrhs
c      RHS(1,j  )=(ac*F(1,j)+bc*F(2,j)+cc*F(3,j))*dxi
      RHS(1,j  )=(-15*F(1,j)+50*F(2,j)/3-0.6*F(3,j)-16./15.*Twb)*dxi
      enddo
      A  (N+1  )=15.
      B  (N+1  )=1.
      C  (N+1  )=0.
      do j=1,nrhs
       RHS(N+1,j  )=(15*f(n,j)-50*f(n-1,j)/3+0.6*f(n-2,j)+16./15.*Twt)*dxi
      enddo
