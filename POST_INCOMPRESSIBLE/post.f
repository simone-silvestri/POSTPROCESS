      ...
      open(5,file='IN')
      read(5,*)       nfiles,iskip,Re,Pr,Pl,Tplus,Tc,Lz
            close(5)
      if (nrank.eq.0) then
      write(*,222)  nfiles,iskip,Re,Pr,Pl,Lz,Tc
...
!         fxmtmp(i)=fxmtmp(i)+fxnew(i,j,k)
!         fymtmp(i)=fymtmp(i)+fynew(i,j,k)
!         fzmtmp(i)=fzmtmp(i)+fznew(i,j,k)
!         qmtmp(i)=qmtmp(i)+qrad(i,j,k)
!         Gmtmp(i)=Gmtmp(i)+Gnew(i,j,k)
!         emtmp(i)=emtmp(i)+enew(i,j,k)
        enddo
       enddo
      enddo
       pmtmp=pmtmp/(jmax*kmax)
!       qmtmp=qmtmp/(jmax*kmax)
!       Gmtmp=Gmtmp/(jmax*kmax)
!       emtmp=emtmp/(jmax*kmax)
!       fxmtmp=fxmtmp/(jmax*kmax)
!       fymtmp=fymtmp/(jmax*kmax)
!       fzmtmp=fzmtmp/(jmax*kmax)

       call mpi_allreduce(pmtmp,mpi_sm,imax,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ier)
       pmtmp = mpi_sm
!       call mpi_allreduce(fxmtmp,mpi_sm,imax,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ier)
!       fxmtmp = mpi_sm
!       call mpi_allreduce(fymtmp,mpi_sm,imax,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ier)
!       fymtmp = mpi_sm
!       call mpi_allreduce(fzmtmp,mpi_sm,imax,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ier)
!       fzmtmp = mpi_sm
!       call mpi_allreduce(qmtmp,mpi_sm,imax,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ier)
!       qmtmp = mpi_sm
!       call mpi_allreduce(Gmtmp,mpi_sm,imax,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ier)
!       Gmtmp = mpi_sm
!       call mpi_allreduce(emtmp,mpi_sm,imax,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ier)
!       emtmp = mpi_sm
     ...
     +        ducfluc,dvcfluc,dwcfluc,ducc,dvcc,dwcc,nfiles)
     ...
     +               nfiles)
      ...
      call parsum5(radm,rdm,rem,dum5,dum5,dum5,dum5)
     ...
     +                       sqrt(Par(i)),sqrt(Per(i)),kGr(i),Pec(i),Pac(i),kc(i),ec(i),em(i),cm(i),km(i),Gm(i)
      ...
      do i=1,imax
       write(36,'(21E16.5)') rp(i),um(i),vm(i),wm(i),cm(i),pm(i),qm(i),em(i),km(i),Gm(i),Pam(i),Pem(i),fxm(i),fym(i),fzm(i),
     +                       uu(i),vv(i),ww(i),uw(i)
      end do
      close(36)
...
!      call decomp_2d_read_one(1,fznew,'BENCH/fz.'//cha2//'.dns')
      call decomp_2d_read_one(1,unew,'DATAgrey/tau10_srefine/u.'//cha2//'.dns')
      call decomp_2d_read_one(1,vnew,'DATAgrey/tau10_srefine/v.'//cha2//'.dns')
      call decomp_2d_read_one(1,wnew,'DATAgrey/tau10_srefine/w.'//cha2//'.dns')
      call decomp_2d_read_one(1,cnew,'DATAgrey/tau10_srefine/c.'//cha2//'.dns')
      call decomp_2d_read_one(1,ptmp,'DATAgrey/tau10_srefine/p.'//cha2//'.dns')
      call decomp_2d_read_one(1,qrad,'DATAgrey/tau10_srefine/q.'//cha2//'.dns')
      call decomp_2d_read_one(1,Gnew,'DATAgrey/tau10_srefine/G.'//cha2//'.dns')
      call decomp_2d_read_one(1,knew,'DATAgrey/tau10_srefine/k.'//cha2//'.dns')
      call decomp_2d_read_one(1,fxnew,'DATAgrey/tau10_srefine/fx.'//cha2//'.dns')
      call decomp_2d_read_one(1,fynew,'DATAgrey/tau10_srefine/fy.'//cha2//'.dns')
      call decomp_2d_read_one(1,fznew,'DATAgrey/tau10_srefine/fz.'//cha2//'.dns')
      endif
      enew=qrad/knew+Gnew
      fxnew=fxnew/pi
      fynew=fynew/pi
